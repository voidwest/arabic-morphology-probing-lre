...
interfaces. The evaluation uses deterministic five-fold outer evaluation with
development-only model selection. It also fixes the Ridge classifier, records
shuffled-label controls, validates token alignment, and archives checksummed
artifacts. Across the 132 model--task--interface comparisons, random-split
macro-F1 is higher than both lemma-heldout and root-heldout macro-F1. The random-minus-lemma differences range from
0.0127 to 0.2863 (mean 0.1059), and random-minus-root differences range from
0.0255 to 0.2796 (mean 0.1179). The size of this difference varies by model,
task, and interface. A single summary penalty therefore obscures the observed
heterogeneity. A full-metadata prompt-final
state compares position together with information availability: the prompt
explicitly supplies lemma, root, and pattern. A metadata-free prompt-final
condition and target-span pooling expose these
...
of its measurement conditions. Those conditions include which examples are
placed together and which information the prompt supplies. They also include
...
in training. To see why this matters, consider the lexical unit shared by the
two sides of a split. Under a random instance split, repeated lemmas, roots, or related
...
comparison between these states therefore changes both position and available
information. We add a metadata-free prompt-final condition to isolate part of
that confound. We retain the complete target span, which allows
final-subtoken and mean-span representations to be derived locally.
...
BF16 matrix. Its contribution is an audited account of the decisions that determine what a
...
\begin{enumerate}
    ...
    report as one penalty, or do they vary across models, tasks, and
    representation interfaces?
    \item \textbf{Reproducibility:} Can complete, validated BF16 captures
    support the reported local analyses using archived captures alone?
\end{enumerate}
...
the two primary models (Qwen3-0.6B and Llama-3.2-1B), all four interfaces, all three split
...
error analysis, and CKA/RSA-compatible matrices.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.96\linewidth]{figures/Fig1_split_deltas.png}
\caption{Mean random-minus-heldout macro-F1 gaps for the eleven models,
averaged over the three tasks and four representation interfaces. The two
bars show lemma-heldout and root-heldout gaps. Model-level means are listed
in Table~\ref{tab:model-deltas}; the complete model-by-task-by-interface values
are available in the archived CSV artifact.}
\label{fig:split-deltas}
\end{figure}
...
\begin{itemize}
    ...
    error analyses, and fixed CKA/RSA diagnostics;
...
\texttt{evidence/bf16-analysis-20260826-v2/}. Historical archives remain
separate, immutable records; this manuscript leaves them unchanged.
...
artifact is available; analyses outside the recorded scope are not inserted
without a corresponding artifact.

\backmatter

\bmhead{Acknowledgements}
During preparation and revision of this manuscript, the author used generative
AI tools, including OpenAI ChatGPT and Codex and DeepSeek-based tools, for
manuscript organization and drafting/revision assistance, as well as software
and documentation support. All outputs were reviewed and verified by the
author, who retains full responsibility for the manuscript, citations, code,
data, analyses, and reported claims.
