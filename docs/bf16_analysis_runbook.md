...
This runbook describes the weight-free analysis created from the validated
BF16 bundles. The committed result is the immutable public freeze at
`evidence/bf16-analysis-20260826-v2/`; new work must use a new run root and
freeze ID.
...
bundles recorded by the current freeze. Every bundle is validated,
checksum-verified, and cross-checked against the 4,701-row frozen dataset
before use.
...
The current full-matrix revision uses five outer folds for every model.
Primary models also receive independent within-partition shuffled-label
controls.
...
OPENBLAS_NUM_THREADS=2 OMP_NUM_THREADS=2 \
  python3 scripts/run_bf16_analysis.py \
  --models qwen3-0.6b --skip-finalize \
  --run-root runs/bf16-analysis/<new-run-id>
```
...
The analysis run root is the source of truth for a new revision. Do not write
to the committed freeze or any historical Q8 material. Freeze a new BF16
analysis with a new dedicated ID after the final audit; the large
representation files remain external artifacts identified by their bundle
checksums.

```bash
python3 scripts/freeze_bf16_analysis.py \
  --run-root runs/bf16-analysis/<new-run-id> \
  --freeze-id <new-freeze-id> \
  --output-root evidence

python3 scripts/verify_bf16_analysis_freeze.py \
  evidence/<new-freeze-id>
```
